...
#     plt.savefig("instance-wise_mask_example7.png")
    ...
    # OPTIMIZED RUN
    # Accuracy reached to 95% with following parameters nd = na = 128, n steps =3, 
    # gamma = 1.5, lambda = 0.001, batch 1024, virtual batch 128
    clf = TabNetClassifier(
        n_d=128,
        n_a=128,
        n_steps=3,
    ...
    clf.fit(
        ...
        eval_set=[(X_train, y_train), (X_valid, y_valid)],
        eval_name=['train', 'valid'],
        max_epochs=10, batch_size=1024, virtual_batch_size=128,
        eval_metric=['accuracy']
    )
    y_pred = clf.predict(X_test)
    test_acc = accuracy_score(y_pred=y_pred, y_true=y_test)
    acc.append(test_acc)
    print("FINAL TEST SCORE FOR forest cover type : {}".format(test_acc))
    n_steps = 3
    explain_matrix, masks = clf.explain(X_test)
    ...
    plt.savefig("optimized_feature_mask_forest_kld100.png")
