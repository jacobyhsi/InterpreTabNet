import torch
from torch.nn import Linear, BatchNorm1d, ReLU, functional
import numpy as np
...
class TabNetEncoder(torch.nn.Module):
    ...
    def reparameterize(self, z, eps:float = 1e-7):
        ...
        s = F.softmax((z + g) / 0.5, dim=-1)
        s = s.view(-1, self.input_dim)
        return s

    def forward(self, x, kls, prior=None):
        x = self.initial_bn(x)
        ...
        with torch.autograd.set_detect_anomaly(True):
            for step in range(self.n_steps):
                ...
                # Third KL Term: compare subsequent masks
                if step == self.n_steps - 1:
                    betas = self.att_transformers[step](prior, att)
                    alphas = self.att_transformers[0](prior, att)
                else:
                    betas = self.att_transformers[step](prior, att)
                    alphas = self.att_transformers[step + 1](prior, att)

                betas = self.reparameterize(betas).cuda()
                alphas = self.reparameterize(alphas).cuda()

                eps = 1e-7
                
                # compare two masks to promote sparsity
                h1 = betas * torch.log(betas + eps)
                h2 = betas * torch.log(alphas + eps)
                kld = torch.mean(torch.sum(h1 - h2, dim=-1), dim=0)

                # Second KL Term 2.0 (Gumble Softmax)
                q_p = betas
                eps = 1e-7
                h1 = q_p * torch.log(q_p + eps)
                h2 = q_p * np.log(1. / self.input_dim + eps)
                kld_PQ = torch.mean(torch.sum(h1 - h2, dim=-1), dim=0)

                # KLD Loss
                M_loss += torch.mean(kld_PQ - self.reg_m*kld)
                # update prior
...
class TabNetNoEmbeddings(torch.nn.Module):
    ...
    def forward(self, x, kls):
        res = 0
        steps_output, M_loss = self.encoder(x, kls)
...
class AttentiveTransformer(torch.nn.Module):
    ...
    def forward(self, priors, processed_feat):
        ...
        x = F.softmax(x, dim=-1)
        # x = self.selector(x)
        # print("within attentive transformer")
        # print(x)
        # print(type(x))
        # mask = torch.where(x == 0, 0.001, 0.0)  # adding a small number to ensure > 0
