...
logs = {} # creates a dictionary to help us log the results
...
class TabModel(BaseEstimator):
    ...
    def __post_init__(self):
        ...
        torch.cuda.set_device(0) # setting cuda used, total 4 GPUs available starting cuda 0
        # print("Device used: cuda", torch.cuda.current_device())
        if self.verbose != 0:
            print(f"Device used cuda: {torch.cuda.current_device()}")
    ...
    def cyclical_annealing_scheduler(self, 
    ...
    def fit(
        ...
        """
        # initialize wandb
        wandb.init(project="tabnet_cVAE", mode="disabled")
        # wandb.init(project="tabnet_cVAE")
        ...
        kl_scheduler = self.cyclical_annealing_scheduler(self.max_epochs)
        ...
        for epoch_idx in range(self.max_epochs):
            ...
            kls = next(kl_scheduler)
            # print("kls:", kls)

            self._train_epoch(train_dataloader, kls)

            # Apply predict epoch to all eval sets
            for eval_name, valid_dataloader in zip(eval_names, valid_dataloaders):
                self._predict_epoch(eval_name, valid_dataloader, kls)
            ...
            wandb.log(logs) # logging the results
    ...
    def predict(self, X):
        ...
        for batch_nb, data in enumerate(dataloader):
            ...
            output, M_loss = self.network(data, None)
    ...
    def _train_epoch(self, train_loader, kls):
        """
        ...
        for batch_idx, (X, y) in enumerate(train_loader):
            ...
            batch_logs = self._train_batch(X, y, kls)
        ...
        logs.update(epoch_logs) # logging the epoch matrix which contains learning rate

        return

    def _train_batch(self, X, y, kls):
        """
        ...
        output, M_loss = self.network(X, kls) # how do we pass in the kl scheduler?

        loss = self.compute_loss(output, y) # recon loss?
        ...
        loss.backward()
        # TODO: this is where gradients are computed
        if self.clip_value:
        ...
        batch_logs["loss"] = loss.cpu().detach().numpy().item() # recording loss
        gradient_logs = {"gradients":gradients} # recording gradients

        logs.update(batch_logs) # wandb updating loss
        logs.update(gradient_logs) # wandb updating gradients
        return batch_logs

    def _predict_epoch(self, name, loader, kls):
        """
        ...
        for batch_idx, (X, y) in enumerate(loader):
            scores = self._predict_batch(X, kls)
            list_y_true.append(y)
            list_y_score.append(scores)

        y_true, scores = self.stack_batches(list_y_true, list_y_score)

        metrics_logs = self._metric_container_dict[name](y_true, scores) # obtaining accuracies

        logs.update(metrics_logs) # wandb updating accuracies
        self.network.train()
        self.history.epoch_metrics.update(metrics_logs)
        return

    def _predict_batch(self, X, kls):
        """
